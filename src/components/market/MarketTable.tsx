...
export default function MarketTable({ items }: MarketTableProps) {
  ...
  return (
    <div className="overflow-hidden rounded-2xl border border-border/70 bg-background/70">
      <div className="grid grid-cols-[2fr_1.2fr_0.8fr_0.8fr_0.8fr] gap-4 border-b border-border/70 px-4 py-3 text-xs uppercase tracking-[0.2em] text-muted-foreground">
        <span>名称</span>
        <span>标签</span>
        <span>评分</span>
        <span>收藏</span>
      ...
      <div className="divide-y divide-border/60" style={{ contentVisibility: 'auto' }}>
        {items.map((item) => (
          <div
            key={item.id}
            className="grid grid-cols-[2fr_1.2fr_0.8fr_0.8fr_0.8fr] items-center gap-4 px-4 py-4 text-sm transition hover:bg-muted/20"
          >
            <div>
